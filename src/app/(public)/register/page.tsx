...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res?.error) {
        setError(res.error.message ?? "Chyba při registraci");
        return;
      }


      setSuccess("Registrace úspěšná. Přesměrovávám na přihlášení...");
      router.push("/login");
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
        <h1 className="text-2xl font-bold mb-6 text-gray-900">Registrace</h1>
